...
<div class="departRoomDetail">
  ...
  <div class="departRoomData flex">
    ...
    <div class="data">
      <span>设备状态:</span>
      <span>{{room.deviceList&&room.deviceList.length?room.deviceList.length:0}}台在线</span>
    </div>
  </div>
  <div class="abnormal">
    <span>异常原因</span>
  ...
  <div :class="['departRoomBtn flex-xy-center',abnormalReason?'':'departRoomBtnNo']" @click="goPoliceRoom">
    ...
    <div class="roomTitle">二级房间</div>
    <div class="roomList flex-y-center" v-for="(item,index) in room.roomList" :key="index">
      <div :class="['text-more-single',items.inWarn?'red':'']" v-for="(items,indexs) in room.roomList[index]" :key="indexs" @click="goTwoRoom(items)">{{items.roomName}}</div>
    </div>
...
export default {
  ...
  methods: {
    async getList () {
      ...
      this.room = result
      if (this.room.roomList && this.room.roomList.length) {
        const roomList = []
        for (let i = 0; i < this.room.roomList.length; i += 4) {
          roomList.push(this.room.roomList.slice(i, i + 4))
        }
        this.room.roomList = roomList
      }
